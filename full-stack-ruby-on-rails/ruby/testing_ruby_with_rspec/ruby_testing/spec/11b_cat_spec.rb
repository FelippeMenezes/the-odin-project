...
describe Cat do
  ...
  subject(:shenna) { described_class.new('Shenna', 'Siamesa') }

  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when cat has method shared with others classes' do
  ...
  it 'is not hungry' do
    expect(shenna).not_to be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(shenna).to be_hiding
  end
end
